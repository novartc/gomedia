...
func makeTrunBox(track *mp4track, start, end int, moofSize uint64) []byte {
	...
	for j := start; j < end; j++ {
		...
		if j+1 < end {
			...
			sampleDuration := track.fallbackSampleDuration()
			if track.lastSample != nil && track.lastSample.dts > track.samplelist[j].dts {
				sampleDuration = uint32(track.lastSample.dts - track.samplelist[j].dts)
			}
			if sampleDuration != track.defaultDuration {
				flag |= TR_FLAG_DATA_SAMPLE_DURATION
			}
		}
	...
	for i := start; i < end; i++ {
		...
		if i == len(track.samplelist)-1 {
			if track.lastSample != nil && track.lastSample.dts > track.samplelist[i].dts {
